...
export default function Navbar({ appointments = [], onSearch }: { appointments?: Appointment[], onSearch: (query: string) => void }) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-4">
                  ...
                  <>
                    ...
                    >
                      <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                        ...
                        <span className="text-xs text-blue-600 font-semibold cursor-pointer hover:underline" onClick={() => handleComingSoon('Tandai Dibaca')}>Tandai dibaca</span>
                      </div>
                        ...
                        {appointments.length === 0 ? (
                          ...
                          <div className="divide-y divide-gray-50">
                            {appointments.map((apt) => (
                              <div key={apt.id} className="p-4 hover:bg-gray-50 transition-colors cursor-pointer group" onClick={() => handleComingSoon('Detail Janji Temu')}>
                      ...
                      {appointments.length > 0 && (
                        <div className="p-3 bg-gray-50 text-center border-t border-gray-100">
                          <button className="text-xs font-bold text-blue-600 hover:underline" onClick={() => handleComingSoon('Daftar Janji Temu')}>Lihat Semua Janji Temu</button>
                        </div>
                      )}
                    </motion.div>
                  </>
            ...
            <button className="p-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors" onClick={() => handleComingSoon('Keranjang')}>
              <ShoppingCart size={20} />
            </button>
            <div className="h-8 w-[1px] bg-gray-200 mx-2 hidden sm:block"></div>
            <button 
              onClick={() => handleComingSoon('Login/Register')}
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-colors shadow-md hover:shadow-lg"
            >
              <User size={18} />
              <span className="text-sm font-medium hidden sm:block">Masuk / Daftar</span>
            </button>
            <button className="p-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors md:hidden" onClick={() => handleComingSoon('Menu')}>
              <Menu size={24} />
            </button>
          </div>
        </div>
